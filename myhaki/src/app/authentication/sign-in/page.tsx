...
export default function SignInPage() {
  const { signin, error, loading } = useFetchSignin();
        ...
        <form className="w-full max-w-sm mx-auto" onSubmit={handleSubmit} noValidate>
            ...
            >
              {!showPassword ? (
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-5.523 0-10-3.582-10-8s4.477-8 10-8c1.27 0 2.485.3 3.613.825m2.512 2.097a9.995 9.995 0 012.503 5.078M3 3l18 18" />
                </svg>
              ) : (
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
         ...
         {message && <div className="mb-4 text-green-600 text-sm text-center">{message}</div>}
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-62 bg-[#A97B5A] ml-16 text-white py-2 rounded mt-2 font-semibold disabled:opacity-50"
          >
